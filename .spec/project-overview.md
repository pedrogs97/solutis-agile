...
- **Investigação e Diagnóstico de Logs**: Skill `log-investigator` e script utilitário `.agents/skills/log-investigator/scripts/extract_logs.py` para extração e análise estruturada de logs das pastas locais de microsserviços e containers Docker (local ou no host Solutis via SSH com diagnóstico de VPN e mascaramento de senhas).
