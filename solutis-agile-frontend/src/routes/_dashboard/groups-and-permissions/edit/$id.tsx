...
// Types mirrored from your data shape
...
function EditGroupPage() {
  const { id } = useParams({
    from: '/_dashboard/groups-and-permissions/edit/$id',
  })
  ...
  const {
    ...
    onSubmit,
  } = useGroupsAndPermissionsDetail({ id })

  // Local search/filter state for permissions
  const [query, setQuery] = useState('')

  // Selected permissions from form state
  ...
  const loading = isFetchingGroup || isFetchingPermissions

  return (
    <>
      <Breadcrumbs />
      <PageSectionHeader title="Editar grupo" />
      ...
      <Grid gutter="lg" align="stretch">
        ...
        <Grid.Col span={{ base: 12, md: 4 }}>
          <Card shadow="sm" p="lg" radius={20} bg={getContentBackgroundColor()}>
            <Stack gap="md">
              <Text c={getSecondaryTextColor()} size="sm">
