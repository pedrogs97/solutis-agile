...
class ProxyService:
    ...
    def _build_authenticated_headers(self, current_user: UserModel) -> Dict[str, str]:
        ...
        return {
            ...
            "x-authenticated-user-full-name": quote(str(full_name).strip(), safe=""),
            "x-authenticated-user-group": quote(str(group_name).strip(), safe=""),
        }
    ...
    async def handle_request_body(self, request: Request):
        ...
        try:
            if content_type.startswith(DEFAULT_MEDIA_TYPE) or content_type.endswith(
                "+json"
            ):
